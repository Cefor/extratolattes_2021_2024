...
# nome_instituicao <- c("Ministério da Educação")
...
# nome_curso <- c("CIÊNCIAS MILITARES, COM ÊNFASE EM GESTÃO OPERACIONAL") # ESAO
# nome_curso <- c("PÓS-GRADUAÇÃO STRICTO SENSU EM EDUCAÇÃO")
...
# fl_maiorestrato: 
# FALSE - pontua artigos pelo estrato da area A1, A2 e B1
# TRUE - pontua artigos pelo maior estrato de A1 a B4
# fl_maiorestrato <- TRUE
...
qualis_periodicos <- import(nm_qualis, encoding = code)
...
# qualis_livros <- read.csv(nm_qualis_livros, header = TRUE, sep = ";", colClasses = "character")
qualis_livros <- import(nm_qualis_livros, encoding = code)
...
# qualis_biblio_autor <- read.csv(nm_qualis_biblio_autor, header = TRUE, sep = ";", colClasses = "character")
qualis_biblio_autor <- import(nm_qualis_biblio_autor, encoding = code)
...
qualis_tecnica <- import(nm_qualis_tecnica, encoding = code)# import eh melhor que read.csv para charset
...
qualis_tecnica_autor <- import(nm_qualis_tecnica_autor, encoding = code)
...
for(cl in 1:nrow(clattes)){
  ...
  data <- xmlParse("curriculo.xml", encoding = code)
