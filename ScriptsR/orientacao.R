...
orientacao <- function(id, xml_data, 
                       pontos_orientacao = function(FUN, item, orientacao, nivel) {FUN(item, orientacao, nivel)},
    ...
    for(i in (1:n)){
      ...
      if((ano >= ano_ini & ano <= ano_fim) | fl_rlvnc=="SIM"){
        ...
        if((instituicao_aux %in% nome_instituicao) & (curso_aux %in% nome_curso)) {
          pontos <- pontos_orientacao(1, tipo, nivel)
        } else pontos <- 0
  ...
  if(n > 0){
    for(i in (1:n)){
      ...
      if(ano >= ano_ini & ano <= ano_fim){
        ...
        if((instituicao_aux %in% nome_instituicao) & (curso_aux %in% nome_curso)) {
          pontos <- pontos_orientacao(2, tipo, nivel)
        } else pontos <- 0
