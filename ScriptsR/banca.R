...
banca <- function(id, xml_data, 
                       pontos_banca = function(FUN, item, nivel) {FUN(item, nivel)},
                       nome_instituicao, nome_curso, ano_ini, ano_fim){
  ...
  if(n > 0){
    for(i in (1:n)){
      ...
      if(ano >= ano_ini & ano <= ano_fim){
        ...
        # pbanca[[i]][[1]] - DADOS-BASICOS-
        # pbanca[[i]][[2]] - DETALHAMENTO-
        ...
        # para considerar cursos de especializacao, inserir em nome_curso em ExtratoLattes.Rmd
        if((instituicao_aux %in% nome_instituicao) & (curso_aux %in% nome_curso)) {
          pontos <- pontos_banca(1, nivel)
        
          
          # pontua quando for outra instituicao/curso
        } else pontos <- pontos_banca(2, nivel)
  ...
  if(n > 0){
    for(i in (1:n)){
      ...
      if(ano >= ano_ini & ano <= ano_fim){
        ...
        if(nivel == "outra" &
           (titulo == "Processo seletivo docente do MPPL" | titulo == "Processo seletivo discente do MPPL") &
           instituicao == "camara dos deputados"){
          curso <- "mestrado profissional em poder legislativo"
          pontos <- pontos_banca(3)

          # quando for outro tipo de banca julgadora
        } else {
          curso <- ""
          pontos <- pontos_banca(4, nivel)
        }
